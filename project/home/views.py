...
def index(request):
    ...
    currently_playing_games = (
        Game.objects.filter(events__user=request.user).distinct()
...
class CalendarView(generic.ListView):
        ...
        # Allow access if the user is the owner or has a valid token
            ...
            if not is_friend:
                if token_user_id == user_id:
                    ...
                    return redirect(
                        reverse("calendar", args=[request.user.id])
    ...
    def get_context_data(self, **kwargs):
        ...
        events = Event.objects.filter(
            ...
            | Q(recurrence_end__gte=d),  # Exclude events with past recurrence_end
        )
...
def event_detail(request, event_id):
    ...
    if request.user.has_perm("view_event", event) | is_friend:
        return render(
            request, "event_detail.html", {"event": event, "is_friend": is_friend, "owner": owner}
        )
    else:
...
def todo_list(request):
    ...
    events = Event.objects.filter(user=request.user, start_time__date=current_date).order_by(
        "game__name", "start_time", "-priority"
    )
    ...
    context = {"games_with_events": sorted_games_with_events}
        ...
        if user_id:
            ...
            if to_user != request.user:
                ...
                if are_already_friends:
                    return JsonResponse(
                        {"success": False, "message": "You are already friends."}, status=400
                    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not friend_request_id:
                return JsonResponse(
                    {"success": False, "error": "Request ID is missing"}, status=400
                )
            ...
            if friend_request.to_user != request.user:
                return JsonResponse(
                    {"error": "You are not authorized to accept this request."}, status=403
                )
...
def decline_friend_request(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Optionally, return a success message or the updated data
            return JsonResponse(
                {"success": True, "message": "Friend request declined successfully."}
            )
...
def ajax_view_friends(request):
    ...
    friends = FriendRequest.objects.filter((Q(from_user=user) | Q(to_user=user)), accepted=True)
    ...
    data = {"friends": [{"username": fr.username, "user_id": fr.id} for fr in friends_list]}
    return JsonResponse(data)
...
def generate_calendar_link(request):
    ...
    if user != request.user:
        return JsonResponse(
            {"success": False, "message": "You are not authorized to share this calendar."}
        )
